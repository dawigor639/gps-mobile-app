...
const [valid, setValid] = useState( 
  ...
  const updateValid = (name,value) => {
    ...
    const updateInterval = (value) => {
      ...
      interval: {
      value: value,
      status: prevState.interval.status
    }}))
    }
    */
    //<CustomTextInput name={"Interval"} type={"int"} maxLength={5} keyboardType={"numeric"} range={{min:120,max:86400}} regex={INTERVAL_REGEX} updateValue={(val) => updateInterval(val)} passValue={element.interval} updateValid={(val) => updateValid("interval",val)} />
